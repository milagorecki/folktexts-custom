...
Usage:
    - exemplary: run_tableshift_benchmark --model gpt2 --results-dir './results/test/' --data-dir '../llm_fairness/folktexts-adapted/data' --task BRFSS_Diabetes --subsampling 0.01 --style "format=bullet,connector=is" --logger-level INFO
"""
import json
import logging
import sys
...
DEFAULT_TABLESHIFT_TASK = "BRFSS_Diabetes"
...
def setup_arg_parser() -> ArgumentParser:
    ...
    # Init parser
    parser = ArgumentParser(description="Benchmark risk scores produced by a language model on Tableshift data.")
    ...
    cli_args = [
        ...
        ("--task",          str, "[str] Name of the TableShift task to run the experiment on", False, DEFAULT_TABLESHIFT_TASK),
...
def main():
    """Prepare and launch the LLM-as-classifier experiment using Tableshift data."""
    ...
    # Fill Tableshift Benchmark config
    ...
    # Create Tableshift Benchmark object
